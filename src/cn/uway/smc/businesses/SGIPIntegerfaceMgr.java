...
public class SGIPIntegerfaceMgr {
	...
	public synchronized int send(Package data) {
		int result = 1;
		try {
	...
	/*
	 * class SendThread extends Thread { // 主函数，执行死循环，进行无限制扫描 public void run()
	 * { while (RunSendFlag) { if ( sat == null ) { sat = new SendAgent(); } if
	 * ( sat.ConnToGWFlag ) { // sat.Submit_MSG(SourceNumber, Charge_Number,
	 * User_Number, // Service_Type, Fee_Type, Fee_Value, MOrelatetoMT_Flag, //
	 * Report_Flag, Message_Content); } else { sat.UnConnectToGW();
	 * sat.Socket_Close(); sat = new SendAgent(); try { sleep(1000); } catch
	 * (Exception eee) { } if ( sat.ConnectToGW() ) { sat.connectflag = true; }
	 * } try { sleep(100000); // 每隔1秒扫一次 } catch (InterruptedException e1) {
	 * System.out.println("程序执行--异常：" + e1.getMessage()); } } } }
	 */
}
